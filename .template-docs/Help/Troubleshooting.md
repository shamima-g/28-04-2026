...
### Lighthouse fails with `CHROME_INTERSTITIAL_ERROR` (Gate 5: Performance)
...
**Cause:** Lighthouse loaded a page that tried to redirect to a backend or auth server that isn't reachable in CI. Most common with BFF authentication (the home page redirects to `http://localhost:5120/authorize` or similar, but no BFF is running in CI) or API-in-development projects (SSR hits a missing API).

**Fix:** The workflow at [.github/workflows/pr-checks.yml](../../.github/workflows/pr-checks.yml) sets `NEXT_PUBLIC_USE_MOCK_API=true` in the performance job so mock handlers short-circuit the backend. If you removed that env var, add it back.

**For real-backend audits:** Deploy to a reachable staging URL, drop `NEXT_PUBLIC_USE_MOCK_API`, and set `LIGHTHOUSE_TARGET_URL` to your staging URL — `web/lighthouserc.js` honours it in place of `http://localhost:3000/`.
